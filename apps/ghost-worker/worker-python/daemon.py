...
# Orchestrator sets inference pricing — daemon fetches /api/pricing (see refresh_pricing_from_backend).
...
def refresh_pricing_from_backend() -> None:
    """Load GHST/output-token rate from orchestrator (single source of truth)."""
    ...
    try:
        ...
        if rate > 0:
            ...
            log({"event": "pricing_sync", "ghst_per_output_token": rate})
