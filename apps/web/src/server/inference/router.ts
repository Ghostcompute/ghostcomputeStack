...
import { JobSubmitSchema, Guarantee, estimatePaymentRaw } from '@ghost-compute/shared';
...
export function createInferenceRouter(orchestrator: Orchestrator) {
  return async function handleChatCompletions(req: any, res: any) {
    ...
    const quotedMaxTokens =
      typeof body.max_tokens === 'number' && body.max_tokens > 0
        ? Math.min(body.max_tokens, 32768)
        : DEFAULT_MAX_OUTPUT_TOKENS;
    const requiredPayment = estimatePaymentRaw(quotedMaxTokens);
    ...
    let tokenBatch = '';
    let tokenFlush: NodeJS.Immediate | null = null;
    ...
    const flushStreamTokens = () => {
      ...
      const chunk = JSON.stringify({
        ...
        choices: [{ index: 0, delta: { content: batch }, finish_reason: null }],
      });
      res.write(`data: ${chunk}\n\n`);
    };

    const onTokenEvent = ({ token }: { token: string }) => {
      if (!isStream || !token) return;
      tokenBuffer.push(token);
      tokenBatch += token;
      if (!tokenFlush) tokenFlush = setImmediate(flushStreamTokens);
    };
    ...
    try {
      const maxTokens = parsed.data.max_tokens ?? 16384;
      ...
      if (isStream) {
        ...
        const streamed = tokenBuffer.join('');
        const full = result.content ?? streamed;
        if (full && !streamedTokens) {
          const chunk = JSON.stringify({
            id: `chatcmpl-${jobId}`,
            object: 'chat.completion.chunk',
            model: parsed.data.model ?? 'ghost',
          ...
          const remainder = full.slice(streamed.length);
          const chunk = JSON.stringify({
            id: `chatcmpl-${jobId}`,
            object: 'chat.completion.chunk',
            model: parsed.data.model ?? 'ghost',
        ...
        const hitLimit = (result.tokensGenerated ?? 0) >= maxTokens;
        if (hitLimit) {
          const chunk = JSON.stringify({
            id: `chatcmpl-${jobId}`,
            object: 'chat.completion.chunk',
