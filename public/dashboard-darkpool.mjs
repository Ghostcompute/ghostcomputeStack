...
async function fetchOrderbook() {
  ...
  const res = await fetch(apiUrl(`/api/orderbook/${base}/${USDC_MINT}`));
  if (!res.ok) throw new Error('HTTP ' + res.status);
  return res.json();
}

async function fetchMatches() {
  const res = await fetch(apiUrl('/api/darkpool/matches'));
  if (!res.ok) throw new Error('HTTP ' + res.status);
  return res.json();
}
...
export async function submitSealedDarkPoolOrder() {
  ...
  const res = await fetch(apiUrl('/api/orders/sealed'), {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      owner_pubkey: owner,
