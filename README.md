...
## Node.js API client (OpenAI-compatible + x402)

Ghost inference uses the same **`POST /v1/chat/completions`** shape as OpenAI, but payment is **GHST via x402** (Solana wallet), not an OpenAI API key.

Use **`@ghost-compute/openai-client`** in this repo:

```typescript
import { createGhostClient, keypairFromSecret } from '@ghost-compute/openai-client';

const client = createGhostClient({
  baseUrl: 'https://api.ghostcompute.tech',
  payer: keypairFromSecret(process.env.GHOST_PAYER_SECRET!),
});
...
The client handles **402 → sign GHST transfer → retry** automatically. See `packages/openai-client/README.md` and `pnpm example:ghost-client`.
